...
// 掃描階段會將每個資料夾嘅 listing 結果快取喺度；
// 複製階段揸嚟用完即刪，避免對同一個資料夾行多一次 list API。
function takeCachedChildren(active, folderId) {
  if (active.childrenCache && active.childrenCache.has(folderId)) {
    const val = active.childrenCache.get(folderId);
    active.childrenCache.delete(folderId);
    return val;
  }
  return null;
}
...
// 順便將每層 listing 結果存落 active.childrenCache，等真正複製嗰陣可以直接攞嚟用，
...
async function runCopyJob(jobId, oauthClient) {
  ...
  try {
    // ---- 掃描階段（只做一次；resume 嘅時候如果之前已經掃描完就跳過；使用者可以選擇唔掃描）----
    if (!job0.scan_enabled) {
      if (job0.scan_status !== 'done') store.setScanStatus(jobId, 'done');
    } else if (job0.scan_status !== 'done') {
...
app.post('/api/copy/start', requireAuth, async (req, res) => {
  const { sourceFolderId, destParentId, newName, mode, scanSize } = req.body;
